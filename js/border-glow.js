...
function animateValue({ start = 0, end = 100, duration = 1000, delay = 0, ease = easeOutCubic, onUpdate, onEnd }) {
  ...
  function tick() {
    ...
    // Only start updating if we are past the delay
    if (elapsed >= 0) {
...
export function initBorderGlow(element) {
  const card = element;
  ...
  const fillOpacity = parseFloat(card.dataset.fillOpacity) || 0.5;
  const animated = card.dataset.animated !== 'false';
  
  let colors = ['#c084fc', '#f472b6', '#38bdf8'];
  if (card.dataset.colors) {
    colors = card.dataset.colors.split(',').map(c => c.trim());
  }
